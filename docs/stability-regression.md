...
两端必须同时使用本次修复后的版本：键鼠协议现为 8（返回与取消控制权的确认、重发），不支持与旧版本混用。以下需要真实双机验证，单元测试不能证明系统休眠、权限和跨屏手感正常。
...
10. Mac 从右侧跨到 Windows，检查 `source_recenter_delta_filtered`：回中后的首次实际位移应连续，Windows 指针不应先跳到左边缘。
11. Windows 控制 Mac 并停在屏内，再移动 Mac 本机鼠标：远端注入造成的光标坐标跳变不应立即触发接管；本机持续移动、点击或按键仍应正常接管。检查 `local_cursor_relocation_filtered` 和 `mouse_local_takeover`。
12. 在两端间制造短时丢包，返回或主动夺回控制权后立即在本机输入；`return` / `cancel` 应持续重发，直到收到 `return_ack` / `cancel_ack`，键盘不能继续发往旧控制端。
13. Windows 在管理员窗口或安全桌面拒绝注入时，应记录 `mouse_simulation_failed` 和 `mouse_incoming_cancelled reason=local_input_injection_failed`，本机鼠标键盘应恢复。观察 `capture_visibility_conflict`，源端不应显示第二个指针。
14. 匀速移动 30 秒并导出两端日志，按会话和序号对比 `udp_signal_sent` / `udp_signal_decrypted`，确认接收缓冲区实际大小、丢包量、源端与接收端的最大间隔。
