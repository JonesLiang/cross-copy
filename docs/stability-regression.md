...
检查了 `~/Library/Application Support/app.crosscopy.desktop/logs/`。最近一次启动为 09:32:39，版本 3.2.8；该次会话记录了 19 次跨屏握手，显示的延迟中位数 5ms、最大 41ms，没有监听器或注入失败记录。该次启动之后没有本机剪贴板日志，因此不能用这些日志证明 Windows 的复制失败原因。
